class AssignsController < ApplicationController
  ...
  before_action :owner_and_current_user_authority, only: [:destroy]
  ...
  def owner_and_current_user_authority
    assign = Assign.find(params[:id])
    if current_user == assign.team.owner || assign.user.id == current_user.id
    else
      redirect_to team_url(params[:team_id]), notice: "リーダー以外による削除はできません。"
    end
  end
end
